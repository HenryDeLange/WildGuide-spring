...
public interface GuideRepository extends CrudRepository<GuideEntity, Long> {
    ...
    List<GuideEntity> findByVisibilityAndName(
        long userId, String name, int limit, int offset);
    
    @Query("SELECT COUNT(g.id) " + FIND_GUIDES)
    int countByVisibilityAndName(
    ...
    @Query("SELECT g.*, "
        ...
        + "FROM \"guides\" g "
        + "WHERE g.id = :guideId")
    GuideEntityExtended findExtendedById(
        long userId, long guideId);
